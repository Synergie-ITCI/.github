...
| --- | --- | --- | --- | --- | --- |
| Token-bearing code execution | Repository commands could run before later framework scripts received `GITHUB_TOKEN` or provider credentials. | Split the reusable workflow into read-only untrusted `detect`/`qa` jobs and a fresh trusted `publisher` job. Each framework checkout uses `job.workflow_repository` at `job.workflow_sha`, and the publisher never checks out PR code. | `.github/workflows/pr-qa.yml` | `tests/test_pr_qa_regressions.py::test_workflow_has_no_framework_override_or_checkout_credentials` | FIXED |
...
- Trusted publisher code comes from `job.workflow_repository` at `job.workflow_sha`.
