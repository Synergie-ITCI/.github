...
| Workflow could check out a stale framework tag | Callers pin the pending immutable `pr-qa-v1.1` release reference, and internal framework checkouts use `job.workflow_repository` at `job.workflow_sha`. Publication must create and protect the tag only after governed review. |
