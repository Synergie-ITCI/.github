...
| Mandatory Gitleaks | CLOSED | Workflow prepares a pinned Gitleaks release before Phase 1; `run_gitleaks()` still fails if unavailable | Regression suite validates the bootstrap contract and fake gitleaks control path | Network access to the approved release asset is required on GitHub-hosted runners. |
